...
namespace AI.SmartStandards.KnowledgeAccess {
  ...
  /// of this interface decide only whether the supplied credentials are valid. They may
  /// use an existing user store, configuration, database, Active Directory integration or
  ...
  public interface IJoplinWebDavAuthenticationValidator {
    ...
    /// <summary>
    /// Validates one username/password pair supplied by the WebDAV client.
    /// </summary>
    ...
    /// </param>
    /// <returns>
    /// <c>true</c> when access to the Joplin WebDAV endpoint is permitted; otherwise
    /// <c>false</c>.
    /// </returns>
  ...
  public class DelegateBasedJoplinWebDavAuthenticationValidator : IJoplinWebDavAuthenticationValidator {
    ...
    private readonly Func<string, string, HttpContext, bool> _validationDelegate;

    public DelegateBasedJoplinWebDavAuthenticationValidator(
      Func<string, string, HttpContext, bool> validationDelegate
    ) {
    ...
    public bool ValidateCredentials(
      ...
      return _validationDelegate(userName, password, context);
    }
